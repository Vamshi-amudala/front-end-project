...
const useForm = ({ resolver }) => {
  ...
  const handleSubmit = (onSubmit) => (e) => {
    ...
    else if (!/\S+@\S+\.\S+/.test(values.email)) newErrors.email = { message: "Invalid email" };

    if (!values.password) newErrors.password = { message: "Password is required" };
    else if (values.password.length < 6) newErrors.password = { message: "Password must be at least 6 characters" };
...
const loginUser = async (data) => {
  ...
  return { role: Math.random() > 0.5 ? "Employer" : "Jobseeker" };
};
    ...
    <div className="relative w-full h-screen flex justify-center items-center overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-emerald-900">
      ...
      <div className="relative z-10 w-full max-w-md bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-8 shadow-2xl backdrop-brightness-75">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 py-3 border border-white/30 hover:bg-white/10 text-white font-medium rounded-xl transition-all duration-300 mb-6"
        >
